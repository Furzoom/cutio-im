...
int main(int argc, char* argv[]) {
  ...
  LogController::Initialize(config.string(), "net-demo", "udp-client", cwd.append("test_tmp").string());

  auto* client = new UDPClientIOEvent;
  bool ret = CreateClientIOService(kIOServiceTypeUDPClient, "", 0, "127.0.0.1", 10001,
                                   client, nullptr, 1, kIOThreadFree, false);

  if (!ret) {
    printf("start failed\n");
  } else {
    printf("connected to 127.0.0.1:10001\n");
  }
